...
def test_read_file_full(tmp_path):
    ...
    result: ToolOutput = read_file_impl(ReadFileInput(path=str(target)))
...
def test_read_file_line_range(tmp_path):
    ...
    result: ToolOutput = read_file_impl(ReadFileInput(path=str(target), offset=2, limit=2))
    assert result.success is True
    data = json.loads(result.content)
...
def test_read_file_tail(tmp_path):
    ...
    result: ToolOutput = read_file_impl(ReadFileInput(path=str(target), tail_lines=3))
    assert result.success is True
    data = json.loads(result.content)
    assert data["content"] == "7\n8\n9"
